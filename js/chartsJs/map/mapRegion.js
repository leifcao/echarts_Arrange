...
echarts.extendsMap = function(id, opt) {
  ...
  var handleEvents = {
    ...
    resetOption: function(i, o, n) {
      ...
      if (j < 0) {
        ...
        if (o.graphic.length > 2) {
          ...
          if(cityData!=null){
            o.series[0].data = handleEvents.initSeriesData(cityData,curGeoJson);
          }else{
            o.series[0].data = [];
          }
        ...
        if (o.graphic.length <= 2) {
          ...
          o.series[0].data = handleEvents.initSeriesData(opt.data,curGeoJson);
        };
        name.splice(j + 1, l);
        idx = j;
    ...
    // 设置effectscatter
    initSeriesData: function(data,Json) {
      var temp = [];
      for (var i = 0; i < data.length; i++) {
        var geoCoord = geoCoordMap[data[i].name];
        if (geoCoord) {
          var list = Json.features.filter((item)=>{
            let name = item.properties.name.slice(0,item.properties.name.length-1)
            return name === data[i].city
            })
          if(list.length){
            temp.push({
              name: data[i].name,
              ...
              level: list[0].properties.level,
              cityCode: list[0].properties.adcode
            });
          }
  ...
  var option = {
    ...
    series: [{
      ...
      data: handleEvents.initSeriesData(opt.data,curGeoJson)
    },{
      name:"map",
      type:'map',
      mapType: 'mapRegion',
  ...
  chart.on('click', function(params) {
    ...
    if (opt.goDown && params.name !== name[idx]) {
      ...
      if (cityCode != parentCode[parentCode.length - 1]) {
        ...
        let url = `data/${rootName.join('')}${cityCode}.json`;
        $.get(url,(response,err)=>{
          ...
          handleEvents.resetOption(_self, option, name);
        }).fail(function() {
          alert('小编正在努力开发中');
          listPop();
        });
...
// mapRegionEcharts.setOption(mapRegionOption,true);
